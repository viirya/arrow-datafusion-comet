...
private[spark] class CometExecRDD(
    sc: SparkContext,
    partitionNum: Int,
    var f: (Seq[Iterator[ColumnarBatch]], Int) => Iterator[ColumnarBatch])
    extends RDD[ColumnarBatch](sc, Nil) {

  override def compute(s: Partition, context: TaskContext): Iterator[ColumnarBatch] = {
    f(Seq.empty, partitionNum)
  }
...
object CometExecRDD {
  def apply(sc: SparkContext, partitionNum: Int)(
      f: (Seq[Iterator[ColumnarBatch]], Int) => Iterator[ColumnarBatch]): RDD[ColumnarBatch] =
    withScope(sc) {
